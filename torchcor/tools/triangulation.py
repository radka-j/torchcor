...
from torchcor.tools.carpmeshreader import CarpMeshReader
from torchcor.tools.carpmeshwriter import CarpMeshWriter


def element_contravariant_basis(
    elemtype: str, VertPts: np.ndarray, localcoords: list = []
):
    ...
    function_dict = {
        "Edges": Edge_contravariant_basis,
...
def Tetrahedron_contravariant_basis(
    VertPts: np.ndarray, localcoords: list = []
) -> dict:
...
class Triangulation:
    ...
    def saveMesh(self, foutName: str):
        ...
        Fibres = self._Fibres
        Elems = self._Elems
        mesh0 = {"Pts": Pts, "Elems": Elems, "Fibres": Fibres}
    ...
    def element_contravariant_basis(
        self, elemtype: str, elemID: int, localcoords: list = []
    ) -> dict:
    ...
    def __compute_contravariant_basis(self) -> dict:
        ...
        for elemtype, Elements in self._Elems.items():
            ...
            for iElem, Elem in enumerate(Elements):
                VertPts = self._Pts[Elem[:-1], :]
                contraBas = element_contravariant_basis(
                    elemtype, VertPts, localcoords=[]
                )
